...
exports.initialize = function(app) {
	...
	var workspacePermission = function(required_permission) {
		...
		var levels = { 'superuser': 1000, 'administrator': 300, 'editor': 200 };
		...
		return function(req, res, next) {
			...
			if (req.session && req.session.is_superuser) {
				...
				/* get workspace permission level to drive display of page controls */
				if (req.session && req.session.username) {
					models.workspace_user_permissions
						.find({ where: { user_id: req.session.user_id, workspace_handle: workspace_handle }})
						.error(req.error)
						.success(function(workspace_user_permission) {
							if (!workspace_user_permission) { return; }
							var permission = Permissions.name(workspace_user_permission.selectedValues.permission_id);
				...
				/* get workspace permission level to determine update permissions */
				models.workspace_user_permissions
					.find({ where: { user_id: req.session.user_id, workspace_handle: workspace_handle }})
					.error(req.error)
					.success(function(workspace_user_permission) {
