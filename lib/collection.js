...
Collection.prototype = {
	...
	initialize: function* (properties, resume) {
		...
		this.fields = yield this._fields(resume());
	...
	save: function* (resume) {
		...
		yield this._data.save().complete(resume());
		this.id = this._data.id;
	},

	update: function* (args, resume) {
		...
		yield this._updateFields({ changes: changes }, resume());
	},

	destroy: function* (args, resume) {
		yield this._data.destroy().complete(resume());
	},

	_fields: function* (resume) {
	...
	_createFields: function* (fields, resume) {
	...
	_updateFields: function* (args, resume) {
...
Collection.create = function* (args, resume) {
	...
	var collection = yield new Collection(properties, resume());
	yield collection.save(resume());
	yield collection._createFields(fields, resume());

	return collection;
};

Collection.load = function* (args, resume) {
...
Collection.all = function* (args, resume) {
...
Collection.itemCounts = function* (args, resume) {
