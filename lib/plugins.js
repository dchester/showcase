...
exports.slurp = function(dir) {
	...
	var route_filename = path.join(dir, 'route.js');
	if (fs.existsSync(route_filename)) {
		var route = require(route_filename);
	}
...
exports.register = function(type, plugin) {
	...
	var views = showcase.app.get('views');

	if (plugin.template) {
		var template_path = path.join(views, 'plugins', type, plugin.config.name);
		swig.compile(plugin.template, { filename: template_path });
		plugin.config.template = template_path;
	}

	if (plugin.route) {
		plugin.route(showcase.app);
	}
};
